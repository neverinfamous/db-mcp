...
- **Code Mode Vector Tests** — Updated `server-instructions/gotchas.md` to document vector tool schema specificities (e.g., `results` instead of `rows`, `value` instead of `distance`, and the requirement of `vectorColumn` for `dimensions`), which deviated from the testing prompt assumptions.
