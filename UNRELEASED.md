...
- Fixed factual errors in `test-codemode-advanced-core.md` test prompt expectations, specifically updating the expected row count for `test_measurements` to 50 rows to reflect `sqlite_read_query` default limits, and correcting `sqlite_drop_table` to expect a success message for non-existent tables due to `IF EXISTS` logic.
- Updated `test-codemode-advanced-core.md` to explicitly append `LIMIT 100` to the `test_events` payload stress test to intentionally bypass the 50-row default limit.
- Fixed `sqlite_list_triggers` returning bloated 50KB+ payloads by filtering out triggers generated by internal SpatiaLite system tables (e.g. `geometry_columns`).
- Fixed `sqlite_date_add` returning silent `null` values instead of an error when negative intervals (e.g. `-9999 years`) cause the result to fall outside SQLite's supported `0000-01-01` to `9999-12-31` bounds.
...
- **Test Prompt Standardization**: Standardized 41 testing prompts across `test-codemode`, `test-advanced`, and `test-tool-groups` using a custom markdown parser script. Unified the boilerplate instructions, reporting rules, structured error expectations, and payload limits into a single cohesive format.
