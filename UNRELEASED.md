...
- **Input Coercion**: Handled invalid numeric input types gracefully in JSON operations (`sqlite_json_each`, `sqlite_json_query`, `sqlite_json_analyze_schema`, `sqlite_json_storage_info`) by replacing `z.coerce.number()` with `z.preprocess()` for `limit` and `sampleSize` parameters — non-numeric values now silently fall back to defaults instead of producing raw MCP `-32602` validation frames.
