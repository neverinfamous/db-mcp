...
- **Wrong-Type Numeric Coercion** — Fixed a bug where passing invalid numeric strings (e.g., `"abc"`) to optional numeric fields defined with `z.preprocess(coerceNumber, ...)` did not fail validation but instead fell back to the schema default. `coerceNumber` now correctly preserves invalid strings so that Zod's `expected number, received string` validation fails, enforcing the Structured Error Response pattern across the `stats`, `geo`, `vector`, `virtual`, and `admin` groups.
