...
import type { McpServerConfig, TransportType, DatabaseConfig } from './types/index.js';
...
Database Options:
  --sqlite <path>           Add SQLite database
...
Server Options:
  ...
  --tool-filter <filter>    Tool filter string (e.g., "-vector,-geo")
...
Examples:
  db-mcp --sqlite ./data.db
  db-mcp --postgresql postgres://user:pass@localhost/db
  db-mcp --transport http --port 3000 --sqlite ./data.db
  db-mcp --sqlite ./data.db --tool-filter "-vector,-stats"
...
function loadEnvConfig(): Partial<McpServerConfig> {
    ...
    if (databaseUri) {
        ...
        } else if (databaseUri.startsWith('mongodb://') || databaseUri.startsWith('mongodb+srv://')) {
            databases.push({ type: 'mongodb', connectionString: databaseUri });
        } else if (databaseUri.startsWith('redis://') || databaseUri.startsWith('rediss://')) {
            databases.push({ type: 'redis', connectionString: databaseUri });
        } else if (databaseUri.startsWith('sqlserver://') || databaseUri.startsWith('mssql://')) {
