...
18. **Vector tool schemas**: Vector tools use distinct schemas from core tools. E.g., `sqlite.vector.search` returns `results` (not `rows`), `distance` returns `value` (not `distance`), and `dimensions` requires `vectorColumn`.
