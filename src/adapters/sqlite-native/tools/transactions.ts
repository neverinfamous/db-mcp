...
const BeginTransactionSchema = z.object({
    mode: z.enum(['deferred', 'immediate', 'exclusive']).optional().default('deferred')
 ...
 */
export function getTransactionTools(adapter: NativeSqliteAdapter): ToolDefinition[] {
    return [
 ...
 * Begin transaction
 */
function createBeginTransactionTool(adapter: NativeSqliteAdapter): ToolDefinition {
    return {
 ...
 * Commit transaction
 */
function createCommitTransactionTool(adapter: NativeSqliteAdapter): ToolDefinition {
    return {
 ...
 */
function createRollbackTransactionTool(adapter: NativeSqliteAdapter): ToolDefinition {
    return {
 ...
 * Release savepoint
 */
function createReleaseSavepointTool(adapter: NativeSqliteAdapter): ToolDefinition {
    return {
 ...
 */
function createRollbackToSavepointTool(adapter: NativeSqliteAdapter): ToolDefinition {
    return {
 ...
 */
function createExecuteInTransactionTool(adapter: NativeSqliteAdapter): ToolDefinition {
    return {
        ...
        handler: async (params: unknown, _context: RequestContext) => {
            ...
            const results: { statement: string; rowsAffected?: number; error?: string }[] = [];
            let success = true;
