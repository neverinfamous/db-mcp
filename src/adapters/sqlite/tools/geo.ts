...
function createGeoNearbyTool(adapter: SqliteAdapter): ToolDefinition {
    return {
        ...
        handler: async (params: unknown, _context: RequestContext) => {
            ...
            const radiusKm = input.unit === 'miles' ? input.radius * 1.60934 :
                input.unit === 'meters' ? input.radius / 1000 : input.radius;
            const latDelta = radiusKm / 111; // ~111km per degree latitude
            const lonDelta = radiusKm / (111 * Math.cos(input.centerLat * Math.PI / 180));
            ...
            const nearby = (result.rows ?? [])
                .map(row => {
                    ...
                    const distance = haversineDistance(
                        input.centerLat, input.centerLon,
                        lat, lon,
                        input.unit
                    );
