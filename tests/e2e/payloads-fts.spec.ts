 ...
 *
 * Tests run in serial: create custom FTS → search → match_info → rebuild → cleanup.
 */
...
test.describe("Payload Contracts: FTS5", () => {
  ...
  test("sqlite_fts_search returns { success, results[], count }", async ({}, testInfo) => {
    const client = await createClient(getBaseURL(testInfo));
    try {
      const payload = await callToolAndParse(client, "sqlite_fts_search", {
        ftsTable: "test_articles_fts",
      ...
      // Each result should have rank and content fields
  ...
  test("sqlite_fts_search with phrase query", async ({}, testInfo) => {
    ...
    try {
      ...
      expectSuccess(payload);
      expect(Array.isArray(payload.results)).toBe(true);
      // Should match the "Understanding Full-Text Search" article
      const results = payload.results as Record<string, unknown>[];
      expect(results.length).toBeGreaterThan(0);
    } finally {
      await client.close();
  ...
  test("sqlite_fts_match_info returns { success, results[], count } with bm25 ranking", async ({}, testInfo) => {
    const client = await createClient(getBaseURL(testInfo));
    try {
