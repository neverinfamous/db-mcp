...
describe("Core Tools - Convenience", () => {
  ...
  describe("sqlite_batch_insert", () => {
    it("should insert multiple rows successfully", async () => {
      ...
      const selectResult = await adapter.executeReadQuery("SELECT * FROM batch_test ORDER BY id");
  ...
  describe("sqlite_upsert", () => {
    it("should fallback to INSERT OR REPLACE if no conflict columns provided", async () => {
      ...
      const selectResult = await adapter.executeReadQuery("SELECT name FROM upsert_test WHERE id = 1");
    ...
    it("should use ON CONFLICT DO UPDATE when conflict columns provided", async () => {
      ...
      await adapter.executeWriteQuery("INSERT INTO upsert_conflict VALUES (1, 'Alice', 30)");
      ...
      const selectResult = await adapter.executeReadQuery("SELECT * FROM upsert_conflict WHERE id = 1");
      expect(selectResult.rows?.[0]?.["name"]).toBe("Alice Updated");
  ...
  describe("sqlite_count", () => {
    it("should count all rows", async () => {
      ...
      await adapter.executeWriteQuery("INSERT INTO count_test VALUES (1), (2), (3)");
    ...
    it("should count rows with condition", async () => {
      await adapter.executeWriteQuery("CREATE TABLE count_cond (id INTEGER, val INTEGER)");
      await adapter.executeWriteQuery("INSERT INTO count_cond VALUES (1, 10), (2, 20), (3, 10)");
  ...
  describe("sqlite_truncate", () => {
    it("should delete all rows from a table", async () => {
      ...
      const selectResult = await adapter.executeReadQuery("SELECT * FROM trunc_test");
