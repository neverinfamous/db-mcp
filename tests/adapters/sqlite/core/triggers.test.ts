...
describe("Core Tools - Triggers", () => {
  ...
  describe("sqlite_list_triggers", () => {
    beforeEach(async () => {
      await adapter.executeWriteQuery(
        "CREATE TABLE test_table (id INTEGER, val TEXT)",
      );
      await adapter.executeWriteQuery(`
    ...
    it("should list all triggers", async () => {
      const res = (await tools.get("sqlite_list_triggers")?.({})) as {
        success: boolean;
        triggers: any[];
      };
      ...
      const updateTrigger = res.triggers.find(
        (t: any) => t.name === "update_val",
      );
    ...
    it("should list triggers for specific table", async () => {
      ...
      await adapter.executeWriteQuery(
        "CREATE TRIGGER other_trig AFTER DELETE ON other_table BEGIN SELECT 1; END;",
      );

      const res = (await tools.get("sqlite_list_triggers")?.({
        table: "test_table",
      })) as { success: boolean; triggers: any[] };
      expect(res.success).toBe(true);
  ...
  describe("sqlite_create_trigger", () => {
    beforeEach(async () => {
      await adapter.executeWriteQuery(
        "CREATE TABLE test_create (id INTEGER, ts TEXT)",
      );
      await adapter.executeWriteQuery(
        "CREATE VIEW test_view AS SELECT * FROM test_create",
      );
    });
  ...
  describe("sqlite_drop_trigger", () => {
    beforeEach(async () => {
      ...
      await adapter.executeWriteQuery(
        "CREATE TRIGGER to_drop AFTER DELETE ON test_drop BEGIN SELECT 1; END;",
      );
    });

    it("should require name", async () => {
      const res = (await tools.get("sqlite_drop_trigger")?.({})) as {
        success: boolean;
      };
      expect(res.success).toBe(false);
    });
    ...
    it("should handle nonexistent trigger", async () => {
      const res = (await tools.get("sqlite_drop_trigger")?.({
        name: "nonexistent",
      })) as { success: boolean; error: string };
      expect(res.success).toBe(false);
      expect(res.error).toContain("does not exist");
    ...
    it("should drop trigger successfully", async () => {
      ...
      const triggerCheck = await adapter.executeReadQuery(
        "SELECT * FROM sqlite_master WHERE type='trigger' AND name='to_drop'",
      );
