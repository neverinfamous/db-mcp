...
describe("Core Tools - Constraints", () => {
  ...
  describe("sqlite_list_constraints", () => {
    ...
    it("should list primary key constraints", async () => {
      await adapter.executeWriteQuery("CREATE TABLE test_pk (id INTEGER, name TEXT, PRIMARY KEY(id, name))");
      const res = await tools.get("sqlite_list_constraints")?.({
        table: "test_pk"
    ...
    it("should list unique index constraints", async () => {
      await adapter.executeWriteQuery("CREATE TABLE test_uniq (id INTEGER, email TEXT UNIQUE, username TEXT)");
      await adapter.executeWriteQuery("CREATE UNIQUE INDEX idx_username ON test_uniq(username)");
    ...
    it("should parse CHECK constraints", async () => {
      ...
      expect(res.checkConstraints).toContain("status IN ('active', 'inactive')");
    });
