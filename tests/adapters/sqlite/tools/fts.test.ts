...
describe("FTS Tools", () => {
  ...
  describe("FTS tools registration (native adapter)", () => {
    ...
    it("should NOT include FTS tools in shared WASM tool index", async () => {
      const { getAllToolDefinitions } = await import("../../../../src/adapters/sqlite/tools/index.js");
      const sharedTools = getAllToolDefinitions(
        adapter as unknown as import("../../../../src/adapters/sqlite/sqlite-adapter.js").SqliteAdapter,
      );
      const ftsTools = sharedTools.filter((t: { name: string }) => t.name.includes("fts"));
  ...
  describe("FTS Execution", () => {
    beforeEach(async () => {
      await adapter.executeWriteQuery(`CREATE TABLE documents (id INTEGER PRIMARY KEY, title TEXT, body TEXT)`);
      await adapter.executeWriteQuery(`INSERT INTO documents (title, body) VALUES ('First Post', 'Hello world, this is a test.')`);
      await adapter.executeWriteQuery(`INSERT INTO documents (title, body) VALUES ('Second Post', 'FTS5 is a powerful full-text search engine.')`);
    });
    ...
    it("should search FTS table", async () => {
      // First create
      await tools.get("sqlite_fts_create")?.({ ftsTable: "documents_fts", sourceTable: "documents", columns: ["title", "body"] });
      
      const searchAll = (await tools.get("sqlite_fts_search")?.({ table: "documents_fts", query: "*", limit: 10 })) as any;
      expect(searchAll.success).toBe(true);
      expect(searchAll.rowCount).toBeGreaterThan(0);

      const searchSpecific = (await tools.get("sqlite_fts_search")?.({ table: "documents_fts", query: "powerful", highlight: true, limit: 10 })) as any;
    ...
    it("should search with column filters", async () => {
      await tools.get("sqlite_fts_create")?.({ ftsTable: "documents_fts", sourceTable: "documents", columns: ["title", "body"] });
      const searchSpecific = (await tools.get("sqlite_fts_search")?.({ table: "documents_fts", query: "Hello", columns: ["body"], limit: 10 })) as any;
      expect(searchSpecific.success).toBe(true);
    ...
    it("should rebuild FTS index", async () => {
      await tools.get("sqlite_fts_create")?.({ ftsTable: "documents_fts", sourceTable: "documents", columns: ["title", "body"] });
      const rebuild = (await tools.get("sqlite_fts_rebuild")?.({ table: "documents_fts" })) as any;
      expect(rebuild.success).toBe(true);
    });

    it("should return match info", async () => {
      await tools.get("sqlite_fts_create")?.({ ftsTable: "documents_fts", sourceTable: "documents", columns: ["title", "body"] });
      const matchInfo = (await tools.get("sqlite_fts_match_info")?.({ table: "documents_fts", query: "test", format: "bm25" })) as any;
    ...
    it("should generate headlines", async () => {
      await tools.get("sqlite_fts_create")?.({ ftsTable: "documents_fts", sourceTable: "documents", columns: ["title", "body"] });
      const headline = (await tools.get("sqlite_fts_headline")?.({ table: "documents_fts", query: "engine", column: "body" })) as any;
      ...
      const wildcard = (await tools.get("sqlite_fts_headline")?.({ table: "documents_fts", query: "*", column: "body" })) as any;
