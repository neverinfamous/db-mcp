...
describe("Text Sentiment Tool", () => {
  ...
  describe("sentiment classification", () => {
    it("should classify very positive text", async () => {
      const result = (await sentimentTool({
        text: "This is amazing, wonderful, and absolutely fantastic!",
      })) as SentimentResult;
    ...
    it("should classify mixed sentiment as balanced", async () => {
      ...
      expect(result.success).toBe(true);
      // "great" (+1) and "terrible" (-1) cancel out
      expect(result.sentiment).toBe("neutral");
      expect(result.positiveCount).toBeGreaterThan(0);
  ...
  describe("edge cases", () => {
    ...
    it("should return structured error for empty text", async () => {
      const result = (await sentimentTool({
        text: "",
      })) as SentimentResult;
    ...
    it("should return structured error for whitespace-only text", async () => {
      const result = (await sentimentTool({
        text: "   ",
      })) as SentimentResult;
