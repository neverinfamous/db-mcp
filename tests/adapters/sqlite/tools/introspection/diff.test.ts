...
describe("Introspection Tools - Schema Diff", () => {
  ...
  beforeEach(async () => {
    ...
    await adapter.executeWriteQuery(
      "CREATE TRIGGER trig_users AFTER INSERT ON users BEGIN SELECT 1; END;",
    );
  });
  ...
  describe("sqlite_schema_diff", () => {
    it("should require baseline and target", async () => {
      const res = (await tools.get("sqlite_schema_diff")?.({})) as {
        success: boolean;
        error: string;
      };
    ...
    it("should detect added table and modified severity", async () => {
      ...
      expect(res.sections.tables.added.map((t: any) => t.name)).toContain(
        "new_table",
      );
    });
    ...
    it("should detect removed view and index, severity high", async () => {
      ...
      expect(res.sections.views.removed.map((v: any) => v.name)).toContain(
        "v_users",
      );
      expect(res.sections.indexes.removed.map((i: any) => i.name)).toContain(
        "idx_users_name",
      );
    });
    ...
    it("should detect modified table column", async () => {
      ...
      const usersTableChange = res.sections.tables.modified.find(
        (t: any) => t.name === "users",
      );
      expect(usersTableChange).toBeDefined();
      expect(
        usersTableChange.changes.find(
          (c: any) => c.type === "column_added" && c.column === "age",
        ),
      ).toBeDefined();
    });
    ...
    it("should allow partial section diffs", async () => {
      ...
      expect(res.sections.indexes.added.map((i: any) => i.name)).toContain(
        "idx_partial",
      );
    });
  });
