...
describe('JSON Tools', () => {
    ...
    describe('sqlite_json_insert', () => {
        it('should insert JSON row', async () => {
            ...
            const count = await adapter.executeReadQuery('SELECT COUNT(*) as count FROM profiles');
            expect(count.rows?.[0]?.['count']).toBe(3);
        });

        it('should insert JSON with additional columns', async () => {
            // Create table with extra column
            await adapter.executeWriteQuery('CREATE TABLE test_json (id INTEGER PRIMARY KEY, json_data TEXT, label TEXT)');
