...
describe("WASM Adapter Concurrency", () => {
  ...
  it("serializes concurrent writes", async () => {
    ...
    for (let i = 0; i < 5; i++) {
      promises.push(
        adapter.executeWriteQuery("INSERT INTO users (name) VALUES (?)", [`User${i}`])
      );
    }
    ...
    const finalCount = await adapter.executeReadQuery("SELECT COUNT(*) as count FROM users");
  ...
  it("rawQuery routes to appropriate lock", async () => {
    ...
    const readResult = await adapter.rawQuery("SELECT name FROM users WHERE id = 1");
    expect(readResult.rows).toHaveLength(1);
    expect(readResult.rows![0]!["name"]).toBe("Alice");
    
    const writeResult = await adapter.rawQuery("UPDATE users SET name = 'Alicia' WHERE id = 1");
    expect(writeResult.rowsAffected).toBe(1);
    
    const verifyResult = await adapter.rawQuery("SELECT name FROM users WHERE id = 1");
