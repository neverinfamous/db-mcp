...
describe('SqliteAdapter', () => {
    ...
    describe('executeReadQuery', () => {
        it('should execute SELECT queries', async () => {
            await adapter.executeWriteQuery('CREATE TABLE test (id INTEGER PRIMARY KEY, name TEXT)');
            await adapter.executeWriteQuery("INSERT INTO test (name) VALUES ('Alice')");
        ...
        it('should return column info', async () => {
            await adapter.executeWriteQuery('CREATE TABLE test (id INTEGER, name TEXT)');
            // Insert a row so we have data to return columns for
            await adapter.executeWriteQuery("INSERT INTO test (id, name) VALUES (1, 'test')");
    ...
    describe('executeWriteQuery', () => {
        it('should execute INSERT and return rows affected', async () => {
            ...
            const result = await adapter.executeWriteQuery("INSERT INTO test (name) VALUES ('Bob')");
        ...
        it('should execute UPDATE and return rows affected', async () => {
            ...
            const result = await adapter.executeWriteQuery("UPDATE test SET name = 'Updated' WHERE name = 'Alice'");
        ...
        it('should execute DELETE and return rows affected', async () => {
            ...
            const result = await adapter.executeWriteQuery("DELETE FROM test WHERE name = 'Alice'");
    ...
    describe('getSchema', () => {
        it('should return schema info', async () => {
            await adapter.executeWriteQuery('CREATE TABLE users (id INTEGER PRIMARY KEY, email TEXT)');
    ...
    describe('listTables', () => {
        it('should list tables', async () => {
            await adapter.executeWriteQuery('CREATE TABLE users (id INTEGER PRIMARY KEY)');
            await adapter.executeWriteQuery('CREATE TABLE orders (id INTEGER PRIMARY KEY)');
