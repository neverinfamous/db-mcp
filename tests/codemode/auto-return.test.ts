...
describe("transformAutoReturn", () => {
  ...
  it("should not modify code ending in non-returnable statements", () => {
    ...
    expect(transformAutoReturn("for (let i = 0; i < 10; i++) {}")).toBe(
      "for (let i = 0; i < 10; i++) {}",
    );
  ...
  it("should correctly identify statement boundaries with nested braces", () => {
    expect(transformAutoReturn("const fn = () => { return 1; }; fn()")).toBe(
      "const fn = () => { return 1; };\nreturn fn()",
    );
    expect(
      transformAutoReturn("const arr = [1, 2, 3]; arr.map(x => x * 2)"),
