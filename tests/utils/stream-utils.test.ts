...
import { streamResultRows, STREAM_CHUNK_SIZE } from "../../src/utils/stream-utils.js";
...
vi.mock("../../src/utils/progress-utils.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../../src/utils/progress-utils.js")>();
  return {
    ...actual,
...
describe("streamResultRows", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    mockCtx = { progressToken: "test-token", server: { notification: vi.fn() } as any };
  });
  ...
  it("should stream multiple chunks correctly", async () => {
    ...
    expect(progressUtils.sendProgress).toHaveBeenNthCalledWith(
      1, mockCtx, 1, 3, JSON.stringify(rows.slice(0, 10))
    );
    
    // Check second chunk (10 rows)
    expect(progressUtils.sendProgress).toHaveBeenNthCalledWith(
      2, mockCtx, 2, 3, JSON.stringify(rows.slice(10, 20))
    );
    
    // Check third chunk (5 rows)
    expect(progressUtils.sendProgress).toHaveBeenNthCalledWith(
      3, mockCtx, 3, 3, JSON.stringify(rows.slice(20, 25))
    );
  });

  it("should use STREAM_CHUNK_SIZE default", async () => {
    const rows = Array.from({ length: STREAM_CHUNK_SIZE + 2 }, (_, i) => ({ id: i }));
