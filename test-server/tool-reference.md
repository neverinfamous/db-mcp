...
| :--- | :--- | :--- |
| `sqlite_execute_code` | *N/A (Sandbox Executor)* | Execute JavaScript in a sandboxed environment with access to all SQLite tools via the `sqlite.*` API. Enables complex multi-step operations in a single call with 70–90% token savings. |
