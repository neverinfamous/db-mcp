...
- **Skip FTS5 tools** (items 18-22: `sqlite_fts_create`, `sqlite_fts_search`, `sqlite_fts_rebuild`, `sqlite_fts_match_info`, `sqlite_fts_headline`) — `[NATIVE ONLY]`. These tools are not registered in WASM and direct calls will fail with "unknown tool".
- **Skip FTS5 checklist items** 15-23 — all require FTS5 tools.
- **Skip error item** 28 (`sqlite_fts_search` on nonexistent table) — `[NATIVE ONLY]`.
- **Skip Zod items** 43-47 (FTS5 tools) — `[NATIVE ONLY]`.
- All other items (1-14, 24-27, 29-42) are fully WASM-compatible — 14 text tools work identically.
